<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Edit_cook.ascx.cs" Inherits="GolhaRestaurant_main_.Control.Edit.Edit_cook" %>
      ...
      <td>
        ...
        <LayoutTemplate>
          <table cellpadding="2" width="630px" id="tbl1" runat="server">
            <tr>
              <th colspan="5" > ویرایش دستورالعمل غذاها  </th>
            </tr>
        ...
        <ItemTemplate>
          <td id="Td1" align="right" runat="server">
           <b><a dir="rtl" id="atitle<%#DataBinder.Eval(Container.DataItem, "fid")%>" 
           onmouseover="mouseover(id)" style="color:#83A741;text-decoration:none"  onmouseout="mouseout(id)"
            href="Main_page.aspx?user=<%#DataBinder.Eval(Container.DataItem, "fid")%>&id=cediting&type=<%#DataBinder.Eval(Container.DataItem, "fcategory")%>"><%#DataBinder.Eval(Container.DataItem, "fname")%></a> </b>
             <asp:Button ID="DeleteButton" runat="server" CommandName="Delete" Text="حذف مطلب" CssClass="button" BorderStyle="None"  />
             
              <asp:Label ID="EmpIDLabel" runat="server" Text='<%# Eval("fid") %>' Visible="false"/>
            ...
            <hr color="#cfe0af">
            <br />
            <a style="text-decoration:blink;color:#5AA4BC" href="Main_page.aspx?user=<%#DataBinder.Eval(Container.DataItem, "fid")%>&id=cediting&type=<%#DataBinder.Eval(Container.DataItem, "fcategory")%>">ویرایش</a>
